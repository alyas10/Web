...
# Импортируем базовый класс из base.py чтобы избежать дублирования
from data_loader.base import BaseDataLoader as LoaderBase


class PcapScapyDataLoader(LoaderBase):
    """
    ...
    def _parse_packet(
        ...
        try:
            ...
            # Сначала проверяем ARP (может быть без IP слоя)
            if ARP in packet:
                arp = packet[ARP]
                protocol = "ARP"
